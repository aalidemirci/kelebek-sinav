...
| **Takvim PDF** | Üst makam sınavları nötr dolgu + lejant; TASLAK filigranı; dipnot düzenlenebilir | Ders saatinin **başlangıç saati** basılmıyor (`start` bağlamda var, şablonda yok) · UYGUNDUR bloğunda **tarih yok**, `generated_at` kullanılmıyor · "Okul Zümre Başkanı" imza yeri **asla dolmaz** · antet "Sancaktepe KAYMAKAMLIĞI" — ilçe büyütülmüyor (`letterhead.py`, `tr_upper` var) · birim satırı resmî usulde "<Okul> Müdürlüğü" | "9. Sınıf (3)" parantez etiketsiz; "[UYGULAMA]" → "Uygulamalı"; "EĞİTİM ÖĞRETİM YILI" → "EĞİTİM VE ÖĞRETİM YILI" |
